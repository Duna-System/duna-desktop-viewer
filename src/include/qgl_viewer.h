...
class QGLViewer : public QOpenGLWidget, protected QOpenGLFunctions
{
  Q_OBJECT
public:
  using PointRGBT = pcl::PointXYZRGB; // We will use "Alpha" as intensity.
  ...
  void setCloud(const pcl::PointCloud<pcl::PointXYZI>::ConstPtr &cloud_);
  void setCloud(const typename pcl::PointCloud<PointRGBT>::ConstPtr &cloud_);
  ...
  int pointPicking(int mouse_x, int mouse_y, int width, int height, QMatrix4x4 MVP, pcl::PointCloud<PointRGBT>::Ptr cloud, float min_dist);
...
protected:
  ...
  PointPicker<PointRGBT> m_point_picker;

  pcl::PointCloud<PointRGBT>::Ptr m_cloud;
